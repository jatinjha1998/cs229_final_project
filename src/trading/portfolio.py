...
class StockHolding:
    def __init__(self, cost, num, name=''):
        self.cost = cost
        self.num = num
        self.name = name
...
def make_portfolio(cost_lo=0, num_lo=0, cost_hi=0, num_hi=0,
    ...
    return pandas.DataFrame(data={'cost_lo': cost_lo, 'num_lo': num_lo,
                                  'cost_lo': cost_hi, 'num_hi': num_hi,
                                  'cash': cash, 'total': total}, index=index)



def allocate_stocks(total_amount=1E6,
                    cost_a=16, cost_b=16, transaction_cost=0.5,
    ...
    na = numpy.floor(target_weights[0] * total_amount / 
            (cost_a + transaction_cost))
    s1 = StockHolding(cost_a, na)
    cash = total_amount - s1.total - na * transaction_cost
   
    nb = numpy.floor(cash // (cost_b + transaction_cost))
    s2 = StockHolding(cost_b, nb)
    cash -= (s2.total + nb * transaction_cost)
    return (s1, s2, cash)

def trade_stocks(percent_trade: float, s1: StockHolding,  s2: StockHolding,
                 cash: float, transaction_cost: float=0.16, total=0):
    """Sell off percent_trade * total of the s1 to buy s2"""
    if not (0 < percent_trade < 1):
        raise ValueError('percent_trade must be in (0, 1)')
    ...
    cash += delta_s1 * (s1.cost - transaction_cost)
    s1.num -= delta_s1

    # buy from smaller stock
    delta_s2 = numpy.floor(cash / (s2.cost + transaction_cost))
    cash -= delta_s2 * (s2.cost + transaction_cost)
    s2.num += delta_s2
